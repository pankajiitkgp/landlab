...
from landlab.components.priority_flood_flow_router._d8_utils import calc_slope_to_d8
from landlab.components.priority_flood_flow_router._d8_utils import (
    calc_steepest_d8_slope,
)
from landlab.components.priority_flood_flow_router._d8_utils import find_d8_link
from landlab.components.priority_flood_flow_router._d8_utils import find_d8_node
    ...
    def _accumulate_flow_d8(self, hill_flow=False):
        ...
        if (hill_flow and self._accumulate_flow_hill) or (
            ...
            if hill_flow and self._accumulate_flow_hill:
                save_drainage_area = self._hill_drainage_area
                save_discharge = self._hill_discharges
            else:
                save_drainage_area = self._drainage_area
                save_discharge = self._discharges

            if np.allclose(self.grid.at_node["water__unit_flux_in"], 1.0):
                discharge = self._node_cell_area
            else:
                discharge = (
                ...
                discharge[self.grid.status_at_node != NodeStatus.CORE] = 0.0

            accumulate_at_receiver_node(
                self._node_cell_area,
                receiver_node_at_node,
                nodes=self._sort[::-1],
                out=save_drainage_area,
            )
            accumulate_at_receiver_node(
                discharge,
                receiver_node_at_node,
